...
from config import SCREEN_WIDTH, SCREEN_HEIGHT, DEFAULT_OBJ_SIZE, \
    DEFAULT_PLAYER_HP, DEFAULT_PLAYER_SPEED, BOMB_EXPLODE_RANGE, \
    DEFAULT_ENEMIES_SPEED, BOMB_TIMER, DEFAULT_EVENT_TIMEOUT
...
class Bomb(EngineSprite):
    ...
    def update(self):
        if self.life_timer == BOMB_TIMER * 60:
            self.kill()
            Fire(self.rect.centerx, self.rect.centery)
            for i in range(1, BOMB_EXPLODE_RANGE):
...
class Enemy(EngineSprite):
    def __init__(self):
        ...
        self.rect = self.surf.get_rect(center=(SCREEN_WIDTH - 25,
                                               SCREEN_HEIGHT - 125))
        self.timer = DEFAULT_EVENT_TIMEOUT * 60
    ...
    def update(self):
        ...
        if pygame.sprite.spritecollideany(
            ...
            self.engine.player.damage -= 1
            if self.engine.player.damage == 0:
                self.engine.player.damage = 60
    ...
    def create_enemy(self):
        ...
        if not self.timer:
            Enemy()
            self.timer = DEFAULT_ENEMIES_SPEED * 60
